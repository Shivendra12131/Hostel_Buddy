...
export default function StickyNavbar() {
	...
	const login = useGoogleLogin({
		onSuccess: async tokenResponse => {
			...
			try {
				...
				postDataFromApi('/users/login', { email: userInfo.data.email })
					.then((data) => {
	...
	const handleSignOut = () => {
		...
		setJwtToken(prev => null);
		setUserData(prev => null);
	}
			...
			<Menu>
				...
				<MenuList>
					...
					<MenuItem className="flex items-center gap-2 ">
						...
						<Typography variant="small" className="font-medium" onClick={handleSignOut}>
							Sign Out
						</Typography>
					</MenuItem>
				</MenuList>
	...
	return (
		<Navbar className="sticky top-0 z-10 h-max max-w-full rounded-none px-4 py-2 lg:px-8 lg:py-4">
			<SignUpForm open={open} setOpen={setOpen} userData={user} />
				...
				<div className="flex items-center gap-4">
					...
					<div className={`flex items-center gap-x-1 ${isAuthenticated && 'hidden'}`}>
						...
						<Button
							...
							size="sm"
							className="inline-block bg-[#350145]"
							onClick={login}
						>
							<span>Sign in</span>
						</Button>
					</div>
					{isAuthenticated && <CustomMenu />}
				</div>
			</div>
		</Navbar>
	);
